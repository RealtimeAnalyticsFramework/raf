...
public class SsbQ1_2IT extends TestCase {
  
  public void testSSBQ1_2() {
    ...
    sql.append("  SELECT sum(lo.lo_extendedprice * lo.lo_discount) AS revenue\n")
       ...
       .append("  WHERE  d.d_yearmonth = 'Jan1994'\n")
       .append("         AND lo.lo_discount BETWEEN 4 AND 6\n")
       .append("         AND lo.lo_quantity BETWEEN 26 AND 35\n");
    
    System.out.println("========================= run SSB Q1.2 =========================");
    ...
    try {
      ...
      File file = new File("Q1.2.txt");
      FileOutputStream fos = new FileOutputStream(file);
      fos.write(("Q1.2 " + new DecimalFormat("0.00").format(time)).getBytes());
      fos.close();
      ...
      if (SsbTable.checkRawResult) {
        ...
        for (int row = 0; row < date.getRowCount(); ++ row) {
          String d_yearmonth = date.getValue(row, "d_yearmonth");
          if (d_yearmonth.equals("Jan1994")) {
        ...
        for (int row = 0; row < lineorder.getRowCount(); ++ row) {
          ...
          if (lo_quantity >= 26 && lo_quantity <= 35 && lo_discount >= 4 && lo_discount <= 6 && dateMap.containsKey(lo_orderdate)) {
