...
case43() {
  ...
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case43_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
  ...
  dist/itest/it_rdd_destroy_test 1>ut_result.log
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
...
case44() {
  ...
  export idgs_member_innerPort=7701
  export idgs_member_service_local_store=true
  dist/bin/idgs -c conf/cluster.conf  1>case44_1.log 2>&1 &
  SRV_PID1=$!
  sleep 2
  ...
  export idgs_member_port=8800
  export idgs_member_innerPort=8801
  dist/bin/idgs -c conf/cluster.conf  1>case44_2.log 2>&1 &
  SRV_PID2=$!
  sleep 2
  ...
  export idgs_member_innerPort=9901
  export idgs_member_service_local_store=false
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case44_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
...
case45() {
  ...
  export idgs_member_innerPort=9901
  export idgs_member_service_local_store=false
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case45_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
...
case46() {
  ...
  export idgs_member_innerPort=9901
  export idgs_member_service_local_store=false
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case46_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
  ...
  dist/itest/it_rdd_store_listener_test
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
...
case47() {
  ...
  dist/itest/it_store_schema_test 1>ut_result.log 2>it_case47.log
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
...
case48() {
  ...
  export idgs_member_innerPort=9901
  export idgs_member_service_local_store=false
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case48_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
  ...
  dist/itest/migration_verify 2>case48_migration_1.log || exit $?
  ...
  dist/itest/migration_verify 2>case48_migration_2.log  || exit $?
  ...
  dist/itest/migration_verify 2>case48_migration_3.log || exit $?

  echo "kill servers 2."
  safekill $SRV_PID2
  sleep 3
  
  dist/itest/migration_verify 2>case48_migration_4.log || exit $?
  ...
  dist/itest/migration_verify 2>case48_migration_5.log || exit $?
  
  echo "kill server 3"
  safekill $SRV_PID3
  sleep 3
  
  dist/itest/migration_verify 2>case48_migration_6.log || exit $?
  
  echo "kill server 2"
  safekill $SRV_PID2
  sleep 3
  
  dist/itest/migration_verify 2>case48_migration_7.log || exit $?
...
case49() {
  ...
  export idgs_member_innerPort=9901
  export idgs_member_service_local_store=false
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case49_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
  ...
  dist/itest/sync_verify 2>case49_sync_1.log
  ...
  dist/itest/sync_verify 2>case49_sync_2.log
  ...
  dist/itest/sync_verify 2>case49_sync_3.log

  echo "kill servers 2."
  safekill $SRV_PID2
  sleep 3
  
  dist/itest/sync_verify 2>case49_sync_4.log
  ...
  dist/itest/sync_verify 2>case49_sync_5.log
  
  echo "kill server 3"
  safekill $SRV_PID3
  sleep 3
  
  dist/itest/sync_verify 2>case49_sync_6.log
  
  echo "kill server 2"
  safekill $SRV_PID2
  sleep 3
  
  dist/itest/sync_verify 2>case49_sync_7.log
...
case50() {
  ...
  dist/itest/migration_verify 2>it_case50_0.log || exit $?
  ...
  export idgs_member_innerPort=9901
  export idgs_member_service_local_store=false
  dist/bin/idgs-load -s 1 -p $TPCH_HOME/dbgen -c conf/cluster.conf -m conf/tpch_file_mapper.conf -t 10 1>case50_load.log 2>&1
  RC=$?
  if [ $RC -ne 0 ] ; then
    echo "Abnormal exit (RC=$RC)";
    exit $RC
  ...
  echo "check backup result"
  dist/itest/migration_verify 2>it_case50_1.log || exit $?
  ...
  dist/itest/migration_verify 2>it_case50_2.log || exit $?
  
  echo "truncate store lineitem"
  dist/itest/it_truncate_store_test
  
  echo "check backup"
  dist/itest/migration_verify 2>it_case50_3.log || exit $?
  ...
  echo "check backup result"
  dist/itest/migration_verify 2>it_case50_4.log || exit $?
