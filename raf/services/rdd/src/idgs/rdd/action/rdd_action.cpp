...
RddAction::RddAction() :
    state(INIT), code(RRC_SUCCESS) {
  ClusterFramework& cluster = ::idgs::util::singleton<ClusterFramework>::getInstance();
  uint32_t partitionSize = cluster.getPartitionCount();
...
const std::string& RddAction::getActionOpName() const {
  return actionOpName;
}
...
const RddState RddAction::getState() const {
  return state;
}
...
const std::vector<std::vector<std::string>>& RddAction::getActionResult() const {
  return actionResult;
}
...
void RddAction::setActionOpName(const std::string& actionOpName) {
...
void RddAction::addActionResult(const uint32_t& partition, const std::string& result) {
