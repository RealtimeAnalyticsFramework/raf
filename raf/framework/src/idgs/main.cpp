...
int main(int argc, char* argv[]) {
  ...
  LOG(INFO) << "Default TBB task scheduler thread pool size: " << tbb::task_scheduler_init::default_num_threads();
  tbb::task_scheduler_init init(std::thread::hardware_concurrency() / 2, 1024 * 1024 * 8);
  try {
...
void monitor(Application& app) {
  ...
  /// @todo for debug
//  idgs::net::NetworkModelAsio* network = ::idgs::util::singleton<idgs::actor::RpcFramework>::getInstance().getNetwork();
  static ::idgs::net::NetworkStatistics* stats = ::idgs::util::singleton<idgs::actor::RpcFramework>::getInstance().getNetwork()->getNetworkStatistics();
  static ::idgs::net::InnerTcpServer* innerTcp = ::idgs::util::singleton<idgs::actor::RpcFramework>::getInstance().getNetwork()->getInnerTcpServer();
  static idgs::actor::ActorMessageQueue& queue = ::idgs::util::singleton<idgs::actor::ActorMessageQueue>::getInstance();
