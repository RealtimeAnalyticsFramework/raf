...
public class SsbQ4_2IT extends TestCase {
  
  public void testSSBQ4_2() {
    ...
    System.out.println("========================= run SSB Q4.2 =========================");
    ...
    try {
      ...
      File file = new File("Q4.2.txt");
      FileOutputStream fos = new FileOutputStream(file);
      fos.write(("Q4.2 " + new DecimalFormat("0.00").format(time)).getBytes());
      fos.close();
      ...
      if (SsbTable.checkRawResult) {
        ...
        for (int row = 0; row < part.getRowCount(); ++ row) {
          ...
          if (p_mfgr.equals("MFGR#1") || p_mfgr.equals("MFGR#2")) {
            ...
            partMap.put(p_partkey, p_category);
          }
        }
        ...
        for (int row = 0; row < lineorder.getRowCount(); ++ row) {
          ...
          if (dateMap.containsKey(lo_orderdate) && custMap.containsKey(lo_custkey) && suppMap.containsKey(lo_suppkey) && partMap.containsKey(lo_partkey)) {
            ...
            String p_category = partMap.get(lo_partkey);
            String key = d_year + "_" + s_nation + "_" + p_category;
