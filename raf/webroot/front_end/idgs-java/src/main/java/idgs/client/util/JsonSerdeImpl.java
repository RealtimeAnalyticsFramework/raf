...
public class JsonSerdeImpl implements ProtoSerde {

  private static Log log = LogFactory.getLog(JsonSerdeImpl.class);

  public String serialize(Message msg) {
    return JsonUtil.pbToJson(msg.toBuilder());
  }

  public void deserialize(Message.Builder builder, String str) {
    try {
      JsonUtil.jsonToPb(builder, str);
  ...
  public void deserializeFromByteArray(Message.Builder builder, byte[] array) {
    try {
      JsonUtil.jsonToPb(builder, new String(array));
    } catch (ParseException e) {
      log.error(e.getMessage());
    }
  }
