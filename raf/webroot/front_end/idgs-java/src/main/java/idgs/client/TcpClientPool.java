...
public class TcpClientPool {
  ...
  public void loadClientConfig(String clientCfgFile) throws IOException {
    log.info("Load client config file " + clientCfgFile);
    File file = new File(clientCfgFile);
    do {
      if (file.exists()) {
        break;
      }
      ...
      if ((temppath = System.getenv("IDGS_HOME")) != null) {
        temppath = temppath + "/" + clientCfgFile;
      ...
      URL url = TcpClientPool.class.getClassLoader().getResource(clientCfgFile);
      if (url != null) {
        file = new File(url.getFile());
        break;
      }
      
      log.error("Client config file " + clientCfgFile + " is not found.");
      try {
        throw new IOException("Client config file " + clientCfgFile + " is not found.");
      } catch (IOException e) {
    ...
    try {
      log.info("Load client config file " + file);
  ...
  private void newConnection(int poolSize) {
    ...
    for (int i = 0; i < poolSize; ++i) {
      ...
      try {
        ...
        client = new TcpClient(server.getHost(), port);
