...
bool FieldExtractor::parse(const idgs::pb::Expr& entryExp, const idgs::actor::PbMessagePtr& key,
...
protobuf::PbVariant FieldExtractor::evaluate(ExpressionContext* ctx) const {
  PbVariant var;
  ...
  for (int32_t i = 0; i < parsedFields.size(); ++i) {
    ...
    switch (field->cpp_type()) {
      ...
      case FieldDescriptor::CPPTYPE_STRING: {
        var = reflection->GetString(* msg, field);
//        std::string s;
...
//        var = new std::string(ref.data(), ref.size());
    ...
    var.is_null = !reflection->HasField(*msg, field);
  }

  return var;
}

const google::protobuf::FieldDescriptor::Type& FieldExtractor::getFieldType() const {
  return fieldType;
}
